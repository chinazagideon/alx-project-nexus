...
class AddressService:
    ...
    def validate_address(self, address_data: Dict) -> Dict:
        ...
        try:
            city = City.objects.select_related('state__country').get(
                id=address_data['city_id']
            )
    ...
    def get_address_hierarchy(self, country_id: int = None, state_id: int = None) -> Dict:
        ...
        result['states'] = [
            {'id': s.id, 'name': s.name, 'country_id': s.country_id}
            for s in states
        ]
    ...
    def get_address_statistics(self) -> Dict:
        ...
        stats = {
            ...
            'countries_with_addresses': Address.objects.filter(
                status=True
            ).values_list(
                'city__state__country__name', flat=True
            ).distinct().count()
        }
