...
class JobSearchService:
    ...
    def _build_response(self, paginated_results: Dict, validated_data: Dict, search_time: float) -> Dict:
        ...
        return {
            ...
            'filters_applied': {k: v for k, v in validated_data.items() if v is not None and k not in ['page', 'page_size', 'sort_by', 'sort_order']}
        }
    ...
    def get_search_suggestions(self, query: str, limit: int = 10) -> List[str]:
        ...
        ).values_list('company__name', flat=True).distinct()[:limit//2]
