    ...
    def create_address_from_data(self, address_data):
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'Successfully created address: {address.location}, '
                        f'{city.name}, {state.name}, {country.name}'
                    )
                )
    ...
    def create_address(self, location, city, zip_code, content_type_name=None, object_id=None):
        ...
        if content_type_name and object_id:
            try:
                ...
                self.stdout.write(
                    self.style.WARNING(f'Content type "{content_type_name}" not found. Using default.')
                )
