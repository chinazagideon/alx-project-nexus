...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not user_id:
            self.stdout.write(self.style.ERROR("Please provide --user-id or use --create-test-data"))
            return
    ...
    def create_test_data(self):
        ...
        user, created = User.objects.get_or_create(
            username="testuser",
            defaults={"email": "testuser@example.com", "first_name": "Test", "last_name": "User", "role": "talent"},
        )
        if created:
        ...
        company, created = Company.objects.get_or_create(
            name="Test Company",
            defaults={"description": "A test company for skill matching", "user": user, "contact_details": "test@company.com"},
        )
        if created:
        ...
        city, created = City.objects.get_or_create(name="Test City", defaults={"state": state})
        if created:
        ...
        for skill_name, proficiency, years in user_skills_data:
            ...
            user_skill, created = UserSkill.objects.get_or_create(
                user=user, skill=skill, defaults={"proficiency_level": proficiency, "years_experience": years}
            )
            if created:
    ...
    def test_skill_matching(self, user_id):
        ...
        try:
            ...
            self.stdout.write("\n=== Job Recommendations ===")
            recommendations = SkillMatchingService.get_job_recommendations(user_id=user_id, limit=5, min_match=50)

            for i, rec in enumerate(recommendations, 1):
                self.stdout.write(f'{i}. {rec["job_title"]} at {rec["company_name"]} ' f'(Match: {rec["match_percentage"]}%)')
            ...
            self.stdout.write(self.style.ERROR(f"Error testing skill matching: {str(e)}"))
